...
public class X509Encoder {
    ...
    public void encode() {
        try {
                ...
                // Create signatures
                //XmlSignatureEngine xmlSignatureEngine = new XmlSignatureEngine(linker, certificate.getIdentifierMap().getMap());
    ...
    private void writeCertificates(final String certificateOutputDirectory, final List<Asn1Encodable> certificates, final byte[][] encodedCertificates) throws IOException {
        ...
        for(int i = 0; i < certificates.size(); i++) {
            ...
            if(de.rub.nds.asn1.util.AttributeParser.parseBooleanAttributeOrDefault(certificate, X509Attributes.ATTACH_TO_CERTIFICATE_LIST, false)) {
    ...
    private void writeSingleCertificate(final String certificateOutputDirectory, final Asn1Encodable certificate, final byte[] encodedCertificate) throws IOException {
