...
/**
 * Main class for the mapping of identifiers and the corresponding Asn1Elements Contains a Hashmap Key:= Path of
 * Identifiers (ex.: /root/inter/leaf) Value:= the corresponding Asn1Element behind the path
 *
 * Is dynamically created of a given Asn1Structure (List of Asn1Encodable) and provides an API for accessing specific
...
public class IdentifierMap {
    ...
    /**
     * Creates the Hashmap of the given Asn1Structure (List of Asn1Encodable)
     *
     * @param asn1Encodables
     *                       The List of Asn1Encodable for which the IdentifierMap is created
     */
     ...
     *
     * @param  idPath
     *                The path of identifiers to an ASN1 element of the X509certificate.
     ...
     *
     * @param  id
     *            The identifier of an element, which is searched for.
    ...
    public List<Asn1Encodable> getElementsByID(String id) {
        ...
        List<Asn1Encodable> asn1List = map.entrySet().stream().filter(x -> x.getValue().getIdentifier().equals(id))
     ...
     *
     * @param  type
     *              The type ASN1 element to search.
     * @return      a List of Asn1Encodable objects for the given identifier
     */
    public List<Asn1Encodable> getElementsByType(String type) {
        ...
        List<Asn1Encodable> asn1List = map.entrySet().stream().filter(x -> x.getValue().getType().equals(type))
            .map(x -> x.getValue()).collect(Collectors.toList());
    ...
    /**
     * Returns all Asn1Elements which has the same class or are an instance of the given class
     *
     * @param  asn1Class
     *                   The asn1Class to search, must implement Asn1Encodable
     * @return           a List of Asn1Encodable objects for the given identifier
     */
    public List<Asn1Encodable> getElementsByClass(Class asn1Class) {
        ...
        List<Asn1Encodable> asn1List =
            map.entrySet().stream().filter(x -> asn1Class.isAssignableFrom(x.getValue().getClass()))
                .map(x -> x.getValue()).collect(Collectors.toList());
     ...
     *
     * @param  asn1
     *              The asn1Element which Path is searched
    ...
    public String getIDPathByElement(Asn1Encodable asn1) {
        ...
        Optional<String> idPath =
            map.entrySet().stream().filter(x -> x.getValue() == asn1).map(x -> x.getKey()).findFirst();
    ...
    public List<String> getIDPathsByID(String id) {
        ...
        List<String> idPaths = map.entrySet().stream().filter(x -> x.getValue().getIdentifier().equals(id))
    ...
    public List<String> getIDPathsByType(String type) {
        ...
        List<String> idPaths = map.entrySet().stream().filter(x -> x.getValue().getType().equals(type))
            .map(x -> x.getKey()).collect(Collectors.toList());
    ...
    /**
     * Returns all identifier Paths of Asn1Encodable object which has the same class or are an instance of the given
     * class
     *
     * @param  asn1Class
     *                   The asn1Class to search, must implement Asn1Encodable
    ...
    public List<String> getIDPathsByClass(Class asn1Class) {
        ...
        List<String> idPaths = map.entrySet().stream().filter(x -> asn1Class.isAssignableFrom(x.getValue().getClass()))
            .map(x -> x.getKey()).collect(Collectors.toList());
    ...
    /**
     * Sets the given Asn1Encdoable in the ASN1 structure of the X509Certificate at the position, described in the given
     * path. It overwrites Asn1Encodable object under the given path.
     *
     *
     * @param  path
     *                                   The path where to set the Asn1Encodable.
     * @param  asn1Encodable
     *                                   The Asn1Encodable Object to setElementByIDPath at the given path
     * @throws X509ModificationException
     *                                   if an Exception occurs while modifying the Asn1 strucutre of the X509Certificat
     *
     */
    public void setElementByIDPath(String path, Asn1Encodable asn1Encodable) throws X509ModificationException {
        ...
        if (asn1Parent instanceof Asn1Container) {
            ...
            int index = IntStream.range(0, childrens.size())
                .filter(childIndex -> childrens.get(childIndex).getIdentifier().equals(identifier)).findFirst()
                .orElse(-1);
            ...
            throw new X509ModificationException(
                "setElementByIDPath(): There parent Object is no instance of an Asn1Container " + parentPath);
        }
    }

    /**
     * Removes the Asn1Encodable at the given Identifier Path. The Asn1Encodable must be a child of a Asn1Container.
     *
     * @param  path
     *                                   The path where to remove the Asn1Encodable.
     * @throws X509ModificationException
     *                                   if an Exception occurs while modifying the Asn1 strucutre of the X509Certificat
     *
     */
    public void removeElementByIDPath(String path) throws X509ModificationException {
        ...
        if (map.containsKey(path) == false) {
            throw new X509ModificationException(
                "removeElementByIDPath(): map does not contain an Asn1Encodable at" + path);
        }
        ...
        if (asn1Parent instanceof Asn1Container) {
            ...
            throw new X509ModificationException(
                "removeElementByIDPath(): There parent Object is no instance of an Asn1Container " + parentPath);
        }
    }
     ...
     *
     * @param param
     *              asn1Encodables The List<Asn1Encodable> for which the IdentifierMap is created.
     *
     */
        ...
        if (asn1Encodables != null) {
            for (Asn1Encodable asn1Encodable : asn1Encodables) {
                ...
                if (this.map.containsKey(fullPathIdentifier) == false) {
                    ...
                    throw new RuntimeException("IdentifierMap -> crawlAsn1EncodedContentRecursive(): Identifier "
                        + fullPathIdentifier + " is used more than once!");
                }
