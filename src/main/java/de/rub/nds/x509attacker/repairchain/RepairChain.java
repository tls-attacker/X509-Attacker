    ...
    private static void repairSignAlgoKeyRelation(X509CertificateChain chain, RepairChainConfig repairConfig)
        ...
        if (repairMode == RepairChainConfig.SignAlgoKeyRelationRepairMode.KEY_BASED) {
            ...
            try {
                ...
                KeyType keyTypeSignAlgo = SignatureEngine.getEngineTupelForOID(effSignOID).getKeyType();

                if (!keyType.equals(keyTypeSignAlgo)) {
                    List<EngineTuple> listOfCompatibleEngines = SignatureEngine.getEngineTupelForKeyType(keyType);
                    Random random = RandomHelper.getRandom();
                    String engineOID = listOfCompatibleEngines.get(random.nextInt(listOfCompatibleEngines.size()))
                        .getObjectIdentifierString();
            ...
            try {
                ...
                KeyType keyTypeSignAlgo_cert0 = SignatureEngine.getEngineTupelForOID(effSignOID_cert0).getKeyType();
                ...
                if (certificateChain.size() > 1) {
                    ...
                    KeyType keyTypeSignAlgo_cert1 = SignatureEngine.getEngineTupelForOID(effSignOID_cert1).getKeyType();

                    if (!keyTypeSignAlgo_cert0.equals(keyTypeSignAlgo_cert1)) {

                        List<EngineTuple> listOfCompatibleEngines =
                            SignatureEngine.getEngineTupelForKeyType(keyTypeSignAlgo_cert0);
                        Random random = RandomHelper.getRandom();
                        String engineOID = listOfCompatibleEngines.get(random.nextInt(listOfCompatibleEngines.size()))
                            .getObjectIdentifierString();
