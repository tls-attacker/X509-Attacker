...
public class MimicryEngine {
    ...
    private static void adjustSignature(X509CertificateConfig signatureKeyConfig, X509Certificate certificate,
        ...
        if (selfsigned) {
            ...
            signatureKeyConfig
                    .setDefaultIssuerNamedCurve(X509NamedCurve.getX509NamedCurve(certificate.getEllipticCurve()));
            ...
            if (certificate.getCertificateKeyType() == X509PublicKeyType.RSA) {
                ...
                signatureKeyConfig.setDefaultIssuerRsaPrivateKey(signatureKeyConfig.getRsaPrivateKey());
                signatureKeyConfig.setDefaultIssuerRsaPublicKey(signatureKeyConfig.getRsaPublicExponent());
            }
        }
    ...
    private static void adjustPublicKey(X509CertificateConfig publicKeyConfig, X509Certificate certificate) {
        ...
        publicKeyConfig.setDefaultSubjectNamedCurve(X509NamedCurve.getX509NamedCurve(certificate.getEllipticCurve()));
        if (certificate.getCertificateKeyType() == X509PublicKeyType.RSA) {
            int bitLength = certificate.getPublicKeyContainer().length();
            BigInteger publicExponent = ((RsaPublicKey) certificate.getPublicKeyContainer()).getPublicExponent();
            Pair<RsaPublicKey, RsaPrivateKey> rsaKeys = KeyGenerator.generateRsaKeys(publicExponent, bitLength,
                    new Random());
        ...
        if (certificate.getCertificateKeyType() == X509PublicKeyType.DSA) {
            DsaPublicKey dsaPublicKey = KeyGenerator.generateDsaPublicKey(publicKeyConfig.getDsaPrivateKey(),
        ...
        Asn1PreparatorHelper.prepareAfterContent(certificate.getTbsCertificate().getSubjectPublicKeyInfo());
