...
public class Certificate extends X509Model<Asn1Sequence>{
    ...
    private Certificate(IntermediateAsn1Field intermediateAsn1Field, String identifier)
    {
        ...
        if(intermediateAsn1Field.getChildren().size()== 3)
        {
            ...
            signatureAlgorithm = AlgorithmIdentifier.getInstance(intermediateAsn1Field.getChildren().get(1), "signatureAlgorithm");
            signatureValue = (Asn1PrimitiveBitString) X509Translator.translateSingleIntermediateField(intermediateAsn1Field.getChildren().get(2), Asn1PrimitiveBitStringFT.class , "signatureValue", "");
