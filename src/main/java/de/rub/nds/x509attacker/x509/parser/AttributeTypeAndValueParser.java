...
public class AttributeTypeAndValueParser extends X509ComponentParser<AttributeTypeAndValue> {
    ...
    public void parseContent(PushbackInputStream inputStream) {
        ...
        Asn1ParserHelper.parseAsn1ObjectIdentifier(attributeTypeAndValue.getType(), inputStream);
        ...
        X500AttributeType attributeType =
                X500AttributeType.decodeFromOidBytes(
                        attributeTypeAndValue.getType().getValueAsOid().getEncoded());
        if (attributeType == null) {
            LOGGER.warn(
                    "Unknown AttributeType: {}. Parsing as unknown.",
                    attributeTypeAndValue.getType().getValue().getValue());
            Asn1ParserHelper.parseUnknown(inputStream);
        } else {
            switch (attributeType) {
                    // @formatter:off
                    /**
                     * X520name ::= CHOICE { teletexString TeletexString (SIZE (1..ub-name)),
                     * printableString PrintableString (SIZE (1..ub-name)), universalString
                     * UniversalString (SIZE (1..ub-name)), utf8String UTF8String (SIZE
                     * (1..ub-name)), bmpString BMPString (SIZE (1..ub-name)) }
                     *
                     * <p>The same format also applies to localityName, stateOrProvinceName,
                     * organizationName, OU
                     */
                    // @formatter:on
                case COMMON_NAME:
                ...
                case ORGANISATION_UNIT_NAME:
                    ...
                    Asn1ParserHelper.parseTagNumberOrUnkownField(
                            inputStream, TagClass.UNIVERSAL, tagNumber);
                    break;
                case COUNTRY_NAME:
                    tagNumber =
                            Asn1ParserHelper.canParse(
