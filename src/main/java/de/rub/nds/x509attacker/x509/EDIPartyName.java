...
public class EDIPartyName extends X509Model<Asn1Sequence>{
    ...
    public static EDIPartyName getInstance(IntermediateAsn1Field intermediateAsn1Field, String identifier, boolean implicit){
    ...
    {
        asn1 = (Asn1Sequence) X509Translator.translateSingleIntermediateField(implicit, intermediateAsn1Field, Asn1SequenceFT.class , identifier, type);
        
        int index = 0;
                   
        //nameAssigner - can be optional
        if(intermediateAsn1Field.getChildren().get(index).getTagNumber() == 0) {
            nameAssigner = DirectoryString.getInstance(intermediateAsn1Field.getChildren().get(index++), "nameAssigner");
