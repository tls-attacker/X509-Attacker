...
public class X509CertificateChain {
    ...
    public List<File> writeCertificateChainToFile(String directory, X509CertChainOutFormat outFormat) {
        ...
        // TOOD: to decide if the attribute "attachToCertificateList" on the Asn1Sequence "certificate" should be
        // considerate here too,
        ...
        switch (outFormat) {
            ...
            case INTER_LEAF_CERTS_COMBINED:
                // only if the chain has at least two certificate and there is atleast a leaf certificate and maybe
                // further intermediate certs
                if (certificateChain.size() >= 2) {
                    try {
