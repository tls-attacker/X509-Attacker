...
public class TBSCertificate extends X509Model<Asn1Sequence> {
    ...
    private TBSCertificate(IntermediateAsn1Field intermediateAsn1Field, String identifier) {
        this.asn1 = (Asn1Sequence) X509Translator.translateSingleIntermediateField(intermediateAsn1Field,
        ...
        if (intermediateAsn1Field.getChildren().size() > 1) {
            ...
            if (intermediateAsn1Field.getChildren().get(0).getTagClass() == TagClass.CONTEXT_SPECIFIC.getIntValue()
                && intermediateAsn1Field.getChildren().get(0).getTagNumber() == 0) {
                version = Version.getInstance(intermediateAsn1Field.getChildren().get(index++), "version");
                asn1.addChild(version.asn1);
            }

            // serialNumber
            serialNumber = (Asn1Integer) X509Translator.translateSingleIntermediateField(
                intermediateAsn1Field.getChildren().get(index++), Asn1IntegerFT.class, "serialNumber",
            ...
            subjectPublicKeyInfo = SubjectPublicKeyInfo.getInstance(intermediateAsn1Field.getChildren().get(index++),
                "subjectPublicKeyInfo");
                ...
                switch (intermediateAsn1Field.getChildren().get(index).getTagNumber()) {
                    ...
                    case 3: // extensions
                        explicitExtensions = ExplicitExtensions
                            .getInstance(intermediateAsn1Field.getChildren().get(index++), "explicitExtensions");
                        asn1.addChild(explicitExtensions.asn1);
                        break;

                    default:
                        LOGGER.warn(
                            "Parser Error: TBSCertificate -> Default Case triggerd; no Parser defined for Tag Number: "
                                + intermediateAsn1Field.getChildren().get(index).getTagNumber());
