...
public class DistributionPointName extends X509Model<Asn1Explicit> {
    ...
    {
        asn1 = (Asn1Explicit) X509Translator.translateSingleIntermediateField(intermediateAsn1Field, Asn1ExplicitFT.class , identifier, type);
        ...
        switch(intermediateAsn1Field.getChildren().get(0).getTagNumber())  {
            ...
            case 1: //nameRelativeToCRLIssuer                     
                choice = ExplicitRelativeDistinguishedName.getInstance(intermediateAsn1Field.getChildren().get(0), "nameRelativeToCRLIssuer");
                asn1.addChild(choice.asn1);
                break;

            
            default:
                LOGGER.warn("Parser Error: DistributionPointName -> Default Case triggerd; no Parser defined for Tag Number: " + intermediateAsn1Field.getChildren().get(0).getTagNumber());
        }  
