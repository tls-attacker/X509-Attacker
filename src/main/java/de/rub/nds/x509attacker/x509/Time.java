...
public class Time extends X509Model<Asn1Encodable>{
    ...
    private Time(IntermediateAsn1Field intermediateAsn1Field, String identifier)
    {
        ...
        if(intermediateAsn1Field.getTagNumber() == TagNumber.UTCTIME.getIntValue()) {
            asn1 = (Asn1PrimitiveUtcTime) X509Translator.translateSingleIntermediateField(intermediateAsn1Field, Asn1PrimitiveUtcTimeFT.class , identifier, type);
        }
        else if (intermediateAsn1Field.getTagNumber() == TagNumber.GENERALIZEDTIME.getIntValue()) {
            asn1 = (Asn1PrimitiveGeneralizedTime) X509Translator.translateSingleIntermediateField(intermediateAsn1Field, Asn1PrimitiveGeneralizedTimeFT.class , identifier, type);
        }         
