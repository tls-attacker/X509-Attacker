...
/**
* Represent one X509Certificate with the certificate itself as an Asn1 structure,
* a SignatureInfo with Information how the signature of the certificate is computed,
* and a KeyInfo Element containing the KeyFile of the certificate.
*/
@XmlRootElement
...
public class X509Certificate {
    ...
    /**
     * Converts the intermediateAsn1Field structure of a parsed Certificate into one X509Certificate object
     * Expects a correct defined intermediate strucutre of a certificate.
     * Create corresponding SignatureInfo and KeyInfo Elements.
     *
     * @param intermediateAsn1Fields A List<IntermediateAsn1Field> containing a parsed Intermediate Asn1 Structure of a certificate
     * 
     */
    ...
    {   
        ...
        Asn1Encodable asn1Enc = this.getIdentifierMap().getElementByIDPath("/certificate/tbsCertificate/subjectPublicKeyInfo");
        ...
        //(for example OpenSSL: To build the chain it will first look and take the certificate in the OS library of trusted CA certificates
        ...
        for(String path : attributeTypeAndValuePathList.stream().filter(s -> s.contains("subject")).collect(Collectors.toList()) )
        {
            if(((Asn1ObjectIdentifier) this.getIdentifierMap().getElementByIDPath(path + "/type")).getValue().equals("2.5.4.3"))
            {
                byte[] guidBytes = guid.toString().split("-")[0].getBytes();
                ((Asn1Field) this.getIdentifierMap().getElementByIDPath(path + "/value")).getContent().setModification(new ByteArrayExplicitValueModification(guidBytes));                
            }
        }
    ...
    /**
     * Returns a List of the Asn1Encodables of the certificate.
     ...
     *
     * @return A List<Ans1Encdoable> of certificate, signatureInfo and keyInfo.
     * 
     */
     ...
     * certificate, signatureInfo and keyInfo Objects.
     *
     * @param certificateOnly Flag whether the List<Asn1Encodable> only contains the certificate or
     * certificate, signatureInfo, keyInfos
     * @return A List<Asn1Encodable> of certificate, signatureInfo and keyInfo.
     * 
     */
    ...
    /**
     * Creates and returns the currently correct IdentifierMap.
     * A Hashmap containing each path of identifiers and the corresponding Asn1Element.
     * Containing the certificate, signatureInfo and KeyInfo structure.
     *
     ...
     * A Hashmap containing each path of identifiers and the corresponding Asn1Element
     * 
     * @param certificateOnly Flag whether the IdentifierMap only contains the certificate or
     * certificate, signatureInfo, keyInfos
     * @return The IdentifierMap of the X509Certificate.
     * 
     */
    ...
    /**
     * Creates and returns the currently correct Linker.
     * A map containing all links between two Asn1 Elements. A link is defined with an 
     * Asn1 Attribute and allows to reuse an Asn1 Element at a different position inside the
     * ASN1 structure.
     *
    ...
    /**
     * Computes and set the signature of the Certificate.
     * For the computation it takes the SignatureInfo of the X509Certificate into consideration for information 
     * about which structure should be signed(default: /certificate/tbsCertificate), which AlgorithmOID and Parameters are used 
     * (default: the Infos defined in /certificate/signatureAlgorithm/) and the target where to write 
     * the computed signature value (default: /certificate/tbsCertificate/signatureValue).
     * The used private Key is definied in the parameter KeyInfo
     * 
    ...
    public String getEffectiveSignatureOID()
    {
        ...
        if(signatureOID == null || signatureOID.isEmpty()) {
            try {
                Asn1ObjectIdentifier asn1ObjectIdentifier = (Asn1ObjectIdentifier) getIdentifierMap().getElementByIDPath(signatureInfo.getSignatureAlgorithmOidIdentifier().trim());
