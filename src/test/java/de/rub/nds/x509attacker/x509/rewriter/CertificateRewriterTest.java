...
public class CertificateRewriterTest {
    ...
    public void testRewriteCertificate() {
        ...
        byte[] originalCertificate = ArrayConverter.hexStringToByteArray(
        ...
        parser.parse(
                new ByteArrayInputStream(originalCertificate));
        CertificateRewriter rewriter = new CertificateRewriter();
        rewriter.fixateNonContainerContent(x509Certificate);
        X509CertificatePreparator preparator = new X509CertificatePreparator(x509Certificate, chooser);
        preparator.prepare();
